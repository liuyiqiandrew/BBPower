...
fchain="test/test_out/emcee.npz"
if python -c "import numpy as np; a=np.load('${fchain}'); print('chain shape =', a['chain'].shape); assert a['chain'].size > 0"; then
    echo "Test passed"
else
