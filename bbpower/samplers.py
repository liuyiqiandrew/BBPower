...
def run_emcee(likelihood: Likelihood, config: dict, output_dir: str) -> dict:
    ...
    if not found_file:
        ...
        pos = [likelihood.params.p0 + 1.e-3 * np.random.randn(ndim)
    ...
    return {'chain': sampler.chain, 'names': likelihood.params.p_free_names,
            'time': elapsed}
...
def run_polychord(likelihood: Likelihood, config: dict, output_dir: str) -> Any:
    ...
    output = pypolychord.run_polychord(pc_likelihood, ndim, nder, settings,
                                       pc_prior, dumper)
...
def run_fisher(likelihood: Likelihood, config: dict, output_dir: str) -> tuple[np.ndarray, np.ndarray]:
...
def run_singlepoint(likelihood: Likelihood, config: dict, output_dir: str) -> float:
    ...
    np.savez(out_path,
             chi2=chi2, ndof=len(likelihood.invcov),
             names=likelihood.params.p_free_names)
    print("Chi2:", chi2, len(likelihood.invcov))
    return chi2


def run_timing(likelihood: Likelihood, config: dict, output_dir: str,
               n_eval: int = 300) -> tuple[float, float]:
...
def run_predicted_spectra(likelihood: Likelihood, compsep: Any,
    ...
    if save_npz:
        np.savez(os.path.join(output_dir, 'cells_model.npz'),
                 tracers=tr_names,
                 ls=compsep.ell_b,
                 dls=model_cls)
