...
class BBPlotter(PipelineStage):
                ...
                for p1 in range(2):
                    ...
                    for p2 in p2range:
                        ...
                        if self.config["plot_coadded_total"]:
                            l, cl, cov = self.s_cd_t.get_ell_cl(
                                typ, t1, t2, return_cov=True
                            )
                            msk = l < self.lmx
                            el = np.sqrt(np.fabs(np.diag(cov)))[msk]
                            plt.errorbar(
                                l[msk], cl[msk], yerr=el, fmt="ro", label="Total coadd"
                            )
                            eb = plt.errorbar(
                                l[msk] + 1, -cl[msk], yerr=el, fmt="ro", mfc="white"
                            )
                            eb[-1][0].set_linestyle("--")
                        if self.config["plot_noise"]:
                            l, cl, cov = self.s_cd_n.get_ell_cl(
                                typ, t1, t2, return_cov=True
                            )
                            msk = l < self.lmx
                            el = np.sqrt(np.fabs(np.diag(cov)))[msk]
                            plt.errorbar(
                                l[msk], cl[msk], yerr=el, fmt="yo", label="Noise"
                            )
                            eb = plt.errorbar(
                                l[msk] + 1, -cl[msk], yerr=el, fmt="yo", mfc="white"
                            )
                            eb[-1][0].set_linestyle("--")
                        l, cl, cov = self.s_cd_x.get_ell_cl(
                            typ, t1, t2, return_cov=True
                        )
                        msk = l < self.lmx
                        el = np.sqrt(np.fabs(np.diag(cov)))[msk]
                        plt.errorbar(
                            l[msk], cl[msk], yerr=el, fmt="bo", label="Cross-coadd"
                        )
                        eb = plt.errorbar(
                            l[msk] + 1, -cl[msk], yerr=el, fmt="bo", mfc="white"
                        )
                        eb[-1][0].set_linestyle("--")
                        plt.yscale("log")
    ...
    def add_contours(self) -> None:
        ...
        with self.doc:
            ...
            chain = self.chain["chain"][:, nsamp // 4 :, :].reshape([-1, npar_chain])[
                :, msk_common
            ]
    ...
    def read_inputs(self) -> None:
        ...
        if self.config["plot_likelihood"]:
            ...
            if not self.can_plot_likelihood:
                print(
                    "Skipping likelihood plots: param_chains file does not contain MCMC samples."
                )
