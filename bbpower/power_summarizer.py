...
class BBPowerSummarizer(PipelineStage):
    ...
    def get_covariance_from_samples(
        self,
        v: np.ndarray,
        s: sacc.Sacc,
    ...
    def init_params(self) -> None:
        ...
        for i in range(self.nsplits):
            ...
            listj = list(filter(lambda x: x not in [i], range(self.nsplits)))
            for j in listj:
                if j < i:
                    continue
                first_pairs.append((i, j))
                # ks that aren't j or i
                listk = list(filter(lambda x: x not in [i, j], range(self.nsplits)))
                for k in listk:
                    # l != i,j,k
                    listl = list(
                        filter(lambda x: x not in [i, j, k], range(self.nsplits))
                    )
                    for l in listl:
                        if l < k:
                            continue
    ...
    def check_sacc_consistency(self, s: sacc.Sacc) -> None:
        ...
        if (
            (len(bands) != self.nbands)
            or (len(splits) != self.nsplits)
            or (len(s.tracers) != self.nbands * self.nsplits)
        ):
            raise ValueError("There's something wrong with these SACC tracers")
        if self.n_bpws == 0:
            self.ells, _ = s.get_ell_cl(
                s.data[0].data_type, s.data[0].tracers[0], s.data[0].tracers[1]
            )
    ...
    def bands_splits_pol_iterator(
        self,
    ) -> Iterator[tuple[int, int, int, int, int, int, int, int, str]]:
    ...
    def parse_splits_sacc_file(
        ...
        spectra = np.transpose(
            s.mean[self.inds].reshape(
                [
                    self.nsplits,
                    self.nbands,
                    2,
        ...
        spectra_total = np.einsum("i,ijklmno,j", weights_total, spectra, weights_total)
        ...
        for i_null, (i, j, k, l) in enumerate(self.pairings):
            spectra_nulls[i_null] = (
                spectra[i, k] - spectra[i, l] - spectra[j, k] + spectra[j, l]
            )
        ...
        if get_saccs:
            ...
            for i_null, (i, j, k, l) in enumerate(self.pairings):
                itrb = self.bands_pol_iterator(half=False, with_windows=with_windows)
        ...
        spectra_total = spectra_total.reshape(
            [2 * self.nbands, 2 * self.nbands, self.n_bpws]
        )[np.triu_indices(2 * self.nbands)].flatten()
        spectra_xcorr = spectra_xcorr.reshape(
            [2 * self.nbands, 2 * self.nbands, self.n_bpws]
        )[np.triu_indices(2 * self.nbands)].flatten()
        spectra_noise = spectra_noise.reshape(
            [2 * self.nbands, 2 * self.nbands, self.n_bpws]
        )[np.triu_indices(2 * self.nbands)].flatten()
    ...
    def run(self) -> None:
        ...
        print("Reading data")
        summ = self.parse_splits_sacc_file(
            self.s_splits, get_saccs=True, with_windows=True
        )
        ...
        self.get_covariance_from_samples(
            sim_cd_t, summ["saccs"][0], covar_type=dctyp, off_diagonal_cut=dcord
        )
        self.get_covariance_from_samples(
            sim_cd_x, summ["saccs"][1], covar_type=dctyp, off_diagonal_cut=dcord
        )
        self.get_covariance_from_samples(
            sim_cd_n, summ["saccs"][2], covar_type=dctyp, off_diagonal_cut=dcord
        )
        ...
        self.get_covariance_from_samples(
            sim_null, summ["saccs"][3], covar_type=nctyp, off_diagonal_cut=ncord
        )

        # Save data
        print("Writing output")
        summ["saccs"][0].save_fits(
            self.get_output("cells_coadded_total"), overwrite=True
        )
